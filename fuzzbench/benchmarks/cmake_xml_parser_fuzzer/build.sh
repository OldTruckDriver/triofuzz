...
# Replace the upstream fuzzing harness with a local version that treats XML
# parse failures as non-crashing behavior. The original harness returns 1 when
# ParseFile fails, which makes libFuzzer assert (Res == 0) for most inputs and
# results in repeated "deadly signal" messages and zero coverage.
cd ..
cp "$SRC/xml_parser_fuzzer.cc" Tests/Fuzzing/xml_parser_fuzzer.cc
cd Tests/Fuzzing
