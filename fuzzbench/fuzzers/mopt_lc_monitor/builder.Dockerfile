...
# Install python3 for patching script
...
# Copy and apply the learning cycle monitoring patch
COPY patch_mopt.py /afl/patch_mopt.py
RUN cd /afl && \
    python3 patch_mopt.py MOpt/afl-fuzz.c MOpt/afl-fuzz.c.patched && \
    mv MOpt/afl-fuzz.c MOpt/afl-fuzz.c.orig && \
    mv MOpt/afl-fuzz.c.patched MOpt/afl-fuzz.c

# Build MOpt-AFL with monitoring
RUN cd /afl/MOpt && AFL_NO_X86=1 make && cp afl-fuzz ..
