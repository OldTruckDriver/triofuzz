...
# Ensure a recent LLVM toolchain is available for AFL++ LLVM mode.
# Prefer LLVM 17 from the official apt.llvm.org repository and make it the default.
RUN apt-get update && \
...
RUN cd /afl && \
    ...
    export CC=clang-17 AFL_NO_X86=1 && \
