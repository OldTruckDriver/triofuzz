...
def get_ttb_from_df(df):
    ...
    # Extract bug names from column names (e.g., "SQL012_R" -> "SQL012")
    bugs = set()
    ...
    # Get final counts from the last row (most recent snapshot)
    # This ensures we capture all bugs that were triggered, even if monitor
    # files were missing for some time intervals
    if not df.empty:
        last_row = df.iloc[-1]
        for bug in bugs:
            reached_col = f"{bug}_R"
            triggered_col = f"{bug}_T"
            
            # Check reached
            if reached_col in df.columns:
                final_reached = int(last_row[reached_col])
                if final_reached > 0:
                    # Get the first time when reached > 0 (time to reach)
                    R = df[df[reached_col] > 0]
                    if not R.empty:
                        reached[bug] = int(R.index[0])
                    else:
                        # Fallback: use final count if no time-to-reach found
                        reached[bug] = final_reached
            
            # Check triggered
            if triggered_col in df.columns:
                final_triggered = int(last_row[triggered_col])
                if final_triggered > 0:
                    # Get the first time when triggered > 0 (time to trigger)
                    T = df[df[triggered_col] > 0]
                    if not T.empty:
                        triggered[bug] = int(T.index[0])
                    else:
                        # Fallback: use final count if no time-to-trigger found
                        triggered[bug] = final_triggered
